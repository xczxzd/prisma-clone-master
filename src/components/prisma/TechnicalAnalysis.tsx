...
const WilliamsRCard: React.FC<{ data: WilliamsRData }> = ({ data }) => {
  ...
  const getSignalIcon = () => {
    if (data.signal === 'STRONG_BUY' || data.signal === 'BUY') {
      return <TrendingUp className="w-5 h-5 text-prisma-green" />;
    }
    if (data.signal === 'STRONG_SELL' || data.signal === 'SELL') {
      return <TrendingDown className="w-5 h-5 text-prisma-red" />;
    }
    return <AlertTriangle className="w-5 h-5 text-prisma-yellow" />;
  };

  const getPatternDescription = () => {
    if (data.zone === 'OVERSOLD' && data.pattern === 'W') {
      return '🟢 Padrão W em zona sobrevendida - REVERSÃO DE ALTA PROVÁVEL!';
    }
    if (data.zone === 'OVERBOUGHT' && data.pattern === 'M') {
      return '🔴 Padrão M em zona sobrecomprada - REVERSÃO DE BAIXA PROVÁVEL!';
    }
    if (data.zone === 'OVERSOLD') {
      return '⚠️ Zona sobrevendida - Aguardando formação de padrão W';
    }
    if (data.zone === 'OVERBOUGHT') {
      return '⚠️ Zona sobrecomprada - Aguardando formação de padrão M';
    }
    ...
    <div className={`prisma-card border-2 ${getZoneColor()} col-span-full`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <h3 className="font-bold text-foreground text-lg">📊 Williams %R (7)</h3>
          {getSignalIcon()}
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-3">
        ...
        <div>
          ...
          <p className={`text-lg font-bold ${
            data.zone === 'OVERSOLD' ? 'text-prisma-green' : 
            data.zone === 'OVERBOUGHT' ? 'text-prisma-red' : 
        ...
        <div>
          ...
          <p className={`text-lg font-bold ${
            data.pattern === 'W' ? 'text-prisma-green' : 
            data.pattern === 'M' ? 'text-prisma-red' : 
            'text-muted-foreground'
          }`}>{data.pattern}</p>
        </div>
      </div>
      ...
      <div className="relative h-6 bg-secondary rounded-full overflow-hidden mb-2">
        ...
        />
        <span className="absolute left-1 top-1/2 -translate-y-1/2 text-xs text-prisma-green">-80</span>
        <span className="absolute right-1 top-1/2 -translate-y-1/2 text-xs text-prisma-red">-20</span>
      </div>
...
export const TechnicalAnalysis: React.FC<TechnicalAnalysisProps> = ({ pair }) => {
  ...
  // Generate Williams %R data
  const williamsRData: WilliamsRData = useMemo(() => {
    const value = (random() * 100 - 100); // -100 to 0
  ...
  const indicatorValues = useMemo(() => ({
    ...
    macd: {
      ...
      signal: (random() - 0.5) * 0.01,
      histogram: (random() - 0.5) * 0.005,
    },
    adx: random() * 50 + 10,
    ema50: random() * 0.1 + 1,
    ema200: random() * 0.1 + 0.98,
    bollinger: {
      upper: random() * 0.05 + 1.02,
      middle: 1,
      lower: random() * 0.05 + 0.93,
    },
    volume: {
      current: random() * 1000000 + 500000,
      average: random() * 800000 + 400000,
    },
  }), [pair.id]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Análise Smart Money Concepts (SMC) + Williams %R</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      {aiAnalysis && (
        <div className="prisma-card gradient-purple border-2 border-primary/50">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-bold text-foreground">🤖 Análise PRISMA IA</h3>
            <span className="ml-auto px-3 py-1 bg-primary/20 text-primary rounded-full text-sm font-bold">
              {aiAnalysis.confluenceScore}% Confluência
            </span>
          </div>
          <div className="prose prose-invert max-w-none">
            <pre className="whitespace-pre-wrap text-sm text-foreground bg-secondary/50 p-4 rounded-lg overflow-auto max-h-96">
              {aiAnalysis.analysis}
            </pre>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Baseado na análise de fluxo de Smart Money + Williams %R
            </p>
          </div>
          <div className="text-right">
            <p className={`text-2xl font-bold ${
              williamsRData.signal.includes('BUY') ? 'text-prisma-green' : 
              williamsRData.signal.includes('SELL') ? 'text-prisma-red' : 
              'text-prisma-yellow'
            }`}>
